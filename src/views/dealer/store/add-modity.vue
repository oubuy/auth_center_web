         ...
         <Row style="background:#fff;display: flex;align-items: center;">
            ...
            <Col span="2" class="centerButton">
                <div class="icon-center">
                    ...
                    <!-- <div @click="handlRight" :class="isActive?'rightIcon':''"><Icon type="ios-arrow-forward" /></div>
                     <div @click="handLeft" :class="isRightActive?'rightIcon':''"><Icon type="ios-arrow-back" /></div> -->
                       <!-- <div @click="handlRight"><Icon type="ios-arrow-forward" /></div>
                     <div @click="handLeft"><Icon type="ios-arrow-back" /></div> -->
                     <div @click="handlRight">加入商品</div>
                     <div @click="handLeft">移除商品</div>
                </div>
...
export default {
  ...
  methods: {
    ...
    handleSubmit() {
      ...
      shopAddModity({ storeModityList: arrSubmit }).then(responese => {
        if (responese.data.code == 200) {
          this.$Message.success(responese.data.msg);
            this.rightData=[];
        this.formData.searchValue='';
          // this.$router.go(-1);
            this.$router.push({path:'/dealer/list',query:{storeId:this.$route.query.storeId,type:this.$route.query.type}})
        }
      });
    }
  },
