...
export default {
  ...
  methods: {
    ...
    handleSuccess(res, file, fileList) {
      if (res.status == 200) {
        ...
        fileList[0].url = res.waterImageUrl;
        obj.url = res.imageUrl;
        obj.name = file.name;
        obj.logo = res.imageId;
        // obj.watchUrl = res.waterImageUrl;//水印图
        obj.watchUrl = res.imageUrl; //水印图

        this.$emit("child-logo", obj);
      }
  ...
  mounted() {
    ...
    // let uuid = this.$route.query.addCategoryId;
    let uuid = "";
    if (this.$route.query.addCategoryId != null) {
      uuid = this.$route.query.addCategoryId;
    } else if (this.$route.query.editCategoryId != null) {
      uuid = this.$route.query.editCategoryId;
    }
    this.cateUrl =
